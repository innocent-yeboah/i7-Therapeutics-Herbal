export const BRAND = {
  name: "i7 Therapeutics Herbal",
  tagline: "Holistic healing & herbal wellness",
  location: "Awoshie Last Stop, Accra, Ghana",
  email: "i7therapeutics.herbal@gmail.com",
  phoneDisplay: "+233 552 473 681",
  phoneTel: "+233552473681",
  /** WhatsApp click-to-chat (digits only, no +). Used as fallback when env is unset. */
  whatsappDigits: "233552473681",
  instagram: "https://www.instagram.com/i7therapeutics_massage",
  instagramLabel: "@i7therapeutics_massage",
  siteCredit: {
    label: "Powered by buildwithinnocent.com",
    href: "https://buildwithinnocent.com",
  },
  mapsEmbedUrl:
    "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3970.5!2d-0.25!3d5.605!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zNcKwMzYnMTguMCJOIDDCsDE1JzAwLjAiVw!5e0!3m2!1sen!2sgh!4v1",
  mapsLink: "https://maps.google.com/?q=Awoshie+Last+Stop+Accra",
  /** Production site (used when NEXT_PUBLIC_APP_URL is unset in server contexts if needed). */
  websiteUrl: "https://i7therapeuticsherbal.com",
  /**
   * Default From: for Resend when `RESEND_FROM_EMAIL` is not set.
   * Must match a verified sending domain in Resend (i7therapeuticsherbal.com).
   */
  emailFrom: "hello@i7therapeuticsherbal.com",
};

export const COLORS = {
  primary: "#2E7D32",
  secondary: "#1E3A5F",
  background: "#FFFFFF",
  text: "#333333",
  border: "#E5E7EB",
};

export const LOW_STOCK_THRESHOLD = 5;

export const BOOKING = {
  slotMinutes: 30,
  dayStartHour: 9,
  dayEndHour: 17,
};
